# 🔐 Настройка SSH-туннелей для 3x-UI API

Этот документ описывает, как настроить безопасные SSH-туннели для доступа к 3x-UI API вместо использования самоподписанных SSL-сертификатов.

## 🎯 Цель

- ✅ 3x-UI слушает только на `127.0.0.1` (недоступен из интернета)
- ✅ Бот подключается к API через SSH-туннель
- ✅ Никаких проблем с SSL-сертификатами
- ✅ Максимальная безопасность
- ✅ Автоматическая настройка для нескольких серверов

## 📋 Шаг 1: Настройка 3x-UI на сервере

Убедитесь, что 3x-UI слушает только на localhost:

```bash
# Проверьте текущую конфигурацию
ss -tulpn | grep 38868

# Должно быть: 127.0.0.1:38868
# Если 0.0.0.0:38868 - измените в настройках 3x-UI
```

В настройках 3x-UI установите:
- **Listen IP**: `127.0.0.1` (не `0.0.0.0`)
- **Port**: `38868`

## 🔑 Шаг 2: Создание SSH-ключа для бота
...
# Сгенерируйте SSH-ключ (без пароля)
ssh-keygen -t ed25519 -f ~/fiorevpn/ssh/x3ui_key -N ""

# Скопируйте публичный ключ на сервер с 3x-UI
ssh-copy-id -i ~/fiorevpn/ssh/x3ui_key.pub user@62.133.60.47
```

**Важно**: Замените `user@62.133.60.47` на реального пользователя и IP сервера с 3x-UI.

## 🔒 Шаг 3: Настройка SSH на сервере с 3x-UI
...
## 🛡️ Шаг 4: Настройка Firewall

Закройте порт 3x-UI от внешнего доступа:
...
## 🐳 Шаг 5: Запуск SSH-туннеля на хосте

**ВАЖНО**: SSH-туннель должен быть запущен **на хосте**, а не в контейнере, и слушать на **0.0.0.0** (все интерфейсы), чтобы быть доступным из Docker-контейнера.

### 5.1. Запустите SSH-туннель на хосте:

```bash
# На хосте (не в контейнере!)
# Команда запустится в фоне и не будет выводить ничего - это нормально
ssh -N \
...
# Сохраните PID для управления
SSH_TUNNEL_PID=$!
echo "SSH-туннель запущен с PID: $SSH_TUNNEL_PID"
```

**Ключевой момент**: `0.0.0.0:38868`, а не `127.0.0.1:38868`!

**Проверка, что туннель работает:**

```bash
# На хосте проверьте, что порт слушается на 0.0.0.0
ss -tulpn | grep 38868
# Должно быть: 0.0.0.0:38868 (не 127.0.0.1:38868!)

# Или
netstat -tulpn | grep 38868
```

Если видите `127.0.0.1:38868` вместо `0.0.0.0:38868`, значит туннель запущен неправильно.
...
**Для первого сервера (порт 38868):**
...
**ВАЖНО**: В базе данных должен быть указан **порт туннеля на хосте** (38869), а не порт на VPN сервере (45087)!

В админ-панели измените:
- **Было**: `https://147.45.50.129:45087/.../panel/api`
- **Стало**: `http://127.0.0.1:38869/.../panel/api` ← **порт 38869, не 45087!**

**Почему?**
- `45087` - это порт 3x-UI на VPN сервере (127.0.0.1:45087)
- `38869` - это порт туннеля на хосте (0.0.0.0:38869), через который контейнер подключается
- Код автоматически заменит `127.0.0.1:38869` на `host.docker.internal:38869`
...
-- ВАЖНО: Заменяем на порт туннеля (38869), а не порт VPN сервера (45087)!
UPDATE servers 
...
-- Также обрабатываем случай, если уже указан http://
UPDATE servers 
SET x3ui_api_url = REPLACE(x3ui_api_url, 'http://147.45.50.129:45087', 'http://127.0.0.1:38869')
WHERE x3ui_api_url LIKE '%147.45.50.129:45087%';
```
...
```bash
# Снаружи сервера (должен быть недоступен)
curl http://62.133.60.47:38868
# Должен быть timeout или connection refused
```
...
2. **Проверьте права на ключ на хосте:**
   ```bash
   chmod 600 ssh/x3ui_key
   ls -la ssh/x3ui_key
   ```

3. **Проверьте, что ключ скопирован на сервер с 3x-UI:**
   ```bash
   ssh -i ssh/x3ui_key user@62.133.60.47 "echo 'OK'"
   ```

4. **Проверьте логи при запуске:**
   ```bash
   docker compose -f docker-compose.prod.yml logs core | grep -i "ssh\|туннель"
   ```
   
   Должно быть:
   ```
   🔐 Настройка SSH-туннеля к 3x-UI...
   📋 SSH-ключ найден: /ssh/x3ui_key
   🔍 Проверка SSH-подключения...
   ✅ SSH-подключение успешно
   🚀 Запуск SSH-туннеля...
   ✅ SSH-туннель запущен и порт 38868 слушается
   ```

5. **Проверьте, что туннель работает:**
   ```bash
   ...
   # Проверка доступности API через туннель
   docker compose -f docker-compose.prod.yml exec core curl -s http://127.0.0.1:38868/panel/api/inbounds/list
   ```

6. **Проверьте процесс autossh:**
   ```bash
...
1. Убедитесь, что URL в БД изменен на `http://127.0.0.1:38868`
2. Проверьте, что 3x-UI слушает на `127.0.0.1:38868`
3. Проверьте туннель:
   ```bash
   docker compose -f docker-compose.prod.yml exec core netstat -tuln | grep 38868
   ```
...
- SSH-туннель автоматически переподключается при обрыве (благодаря `Restart=always` в systemd)
- Для каждого сервера используйте отдельный порт на хосте (38868, 38869, и т.д.)
- Код автоматически заменяет `127.0.0.1` на `host.docker.internal` для доступа из контейнера
- Если SSH-туннель не настроен, приложение работает как обычно (без туннеля)

## 🔧 Управление туннелями

```bash
# Статус туннелей
systemctl status x3ui-tunnel-1
systemctl status x3ui-tunnel-2

# Перезапуск
systemctl restart x3ui-tunnel-1
systemctl restart x3ui-tunnel-2

# Остановка
systemctl stop x3ui-tunnel-1
systemctl stop x3ui-tunnel-2

# Просмотр логов
journalctl -u x3ui-tunnel-1 -f
journalctl -u x3ui-tunnel-2 -f

# Проверка портов
ss -tulpn | grep -E "38868|38869"
```
